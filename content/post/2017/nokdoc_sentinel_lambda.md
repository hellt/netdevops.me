...
* testing basic Python Lambda onboarding process powered by Serverless framework
...
# Init
What we are going to _lambdsify_ is a python3 script called **sentinel** which has the following Lambda-related properties:

* uses some non standard packages (i.e. `requests`)
* reads/writes a file.
...
While it might be good to feel the taste of manual Lambda configuration through the AWS Console, I decided to go "everything as a code" way and use the [Serverless](https://serverless.com/) framework to configure and deploy my first Lambda.
...
Once serverless is installed, I started with creating an `aws-python3` service for the nokdoc_sentinel project:
...
# Making lambda instance out of template
I renamed `handler.py` module to `sentinel.py`, also changed the enclosed function' name and deleted redundant code from the template. For starters I kept the sample code just to test that deploying to AWS via serverless actually works.
...
# name of the func in the module
  check:
    # sentinel.check should have been read like 
...
* archived your project as a zip archive and load it to AWS S3
...
Key artifacts that were created can be browsed with AWS CLI:
...
# for that we need to specify the region serverless used at deployment
...
Basically there are two files we dealt with earlier plus `.vscode` dir that a text editor created for its settings. Having `.vscode` in the deployment package actually shows that by default serverless zipped everything in the project dir. You can get in control of this process by using [include/exclude statements](https://serverless.com/framework/docs/providers/aws/guide/packaging#package-configuration).

# Accessing AWS S3 from within a Lambda
It is natural that AWS assumes that Lambdas will be used in a close cooperation with the rest of the AWS family. And for the file storage **AWS S3** is one-stop shop.
...
Now as the permissions are there, we are free to the list a bucket contents and modify the files in it. AWS provides us with the [boto3](https://boto3.readthedocs.io/en/latest/index.html) package as a Python API for AWS services. Moreover, this package comes pre-installed on the [system that is used to spawn the Lambdas](https://docs.aws.amazon.com/lambda/latest/dg/current-supported-versions.html), so you do not need to provide a package.
...
Well, in that case you need to push these packages along with your function' code as a singe _deployment package_, [as official guide says](https://docs.aws.amazon.com/lambda/latest/dg/lambda-python-how-to-create-deployment-package.html) you need to copy packages to the root directory of your function.
...
* you will have to .gitignore these packages directory to keep your packages out of repository
...
Even though a Lambda often assumed as an independent function, a real application you might want to transfer to Lambda quite likely has dependencies on common code. Refer to the ["Writing Shared Code"](https://serverlesscode.com/post/python-3-on-serverless-framework/#writing-shared-code) section of the above mentioned blog post to see how its done.
...
# Summary
With the above explained concepts I successfully transferred **nokdoc-sentinel** script from a standalone cron-triggered module to the AWS Lambda. You can check the project' code and `serverless.yml` file for it [here](https://github.com/hellt/nokdoc-sentinel-lambda).
...
1. [AWS Lamda Developers Guide](https://docs.aws.amazon.com/lambda/latest/dg/welcome.html)
